...
nvstusb_usb_open_device(
  ...
  int res;
  struct libusb_device_handle *handle = libusb_open_device_with_vid_pid(nvstusb_usb_context, 0x0955, 0x7002);

  if (0 == handle)
  {
      handle = libusb_open_device_with_vid_pid(nvstusb_usb_context, 0x0955, 0x0007);
      if (0 == handle)
      {
